...
// Elementos de filtros actualizados
...
tabBtns.forEach(tab => {
    tab.addEventListener('click', () => {
        ...
        // Si es la pestaña de registros, cargar semana actual
        if (tabId === 'registros') {
...
function cambiarVistaFiltro() {
    ...
    if (vista === 'semana') {
        selectorSemana.style.display = 'block';
        ...
        selectorDia.style.display = 'block';
...
function cargarUsuarios() {
    ...
    if (telepizzeros.length === 0) {
        listaUsuarios.innerHTML = '<p style="text-align: center; color: #FFC72C;">No hay usuarios registrados</p>';
        return;
    }
    ...
    telepizzeros.forEach(usuario => {
        ...
        item.innerHTML = `
            <div style="flex: 1;">
                <strong style="color: white; font-size: 1.1rem;">${usuario.nombre}</strong>
                <div style="margin-top: 5px;">DNI: ${usuario.dni}</div>
                <div style="margin-top: 5px;">
                    <span class="puesto-badge ${obtenerClasePuesto(usuario.puesto)}">
                        ${obtenerNombrePuesto(usuario.puesto)}
                    </span>
                </div>
                <div style="margin-top: 5px; font-size: 0.9rem; color: #FFC72C;">
                    <i class="fas fa-clipboard-list"></i> Registros: ${registrosUsuario}
                </div>
            </div>
            <div class="botones-accion">
                <button class="btn btn-danger btn-small" data-dni="${usuario.dni}">
...
// ============================================
// CONTROL DE AUSENCIAS (SIN CAMBIOS)
// ============================================
...
function cargarAusencias() {
    ...
    ordenadas.forEach(ausencia => {
        ...
        item.innerHTML = `
            <div style="flex: 1;">
                <strong style="color: white; font-size: 1.1rem;">${ausencia.nombre}</strong> 
                <span class="puesto-badge ${obtenerClasePuesto(ausencia.puesto)}">
                    ${obtenerNombrePuesto(ausencia.puesto)}
                </span>
                <div style="margin-top: 8px;">
                    <span style="color: #FFC72C;">${icono} ${tipoTexto}</span> - 📅 ${ausencia.fecha}
                </div>
                <div style="margin-top: 5px; color: #E0E0E0;">📝 ${ausencia.motivo}</div>
                <div style="margin-top: 5px; font-size: 0.8rem; color: #9E9E9E;">
                    🕐 Registrado: ${ausencia.fechaRegistro}
                </div>
            </div>
            <div class="botones-accion">
                <button class="btn btn-danger btn-small" data-id="${ausencia.dni}-${ausencia.fecha}">
...
function cargarCorreoGuardado() {
    if (correoGerencia) {
        infoCorreoGuardado.innerHTML = `
            <h3><i class="fas fa-check-circle" style="color: #4CAF50;"></i> Correo Vinculado</h3>
        ...
        infoCorreoGuardado.innerHTML = `
            <h3><i class="fas fa-envelope"></i> Sin correo vinculado</h3>
            <p style="margin: 10px 0;">Vincule un correo para recibir reportes PDF</p>
            <p style="font-size: 0.9rem; color: #FFC72C;">
...
async function enviarPDFPorCorreo(titulo, pdfBlob, correoDestino) {
    try {
        ...
        const templateParams = {
            ...
            pdf_data: base64data,
            pdf_filename: `${titulo.toLowerCase().replace(/ /g, '_')}.pdf`
        };
        ...
        const response = await emailjs.send(
            'default_service',
            'template_g60lzrr', // Asegúrate de que este template soporte adjuntos
            templateParams
        );
...
// ============================================
// INICIALIZAR APLICACIÓN ACTUALIZADA
// ============================================
